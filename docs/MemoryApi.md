...
|[**getOffloadedContent**](#getoffloadedcontent) | **POST** /v2/memory/node/{node_id}/offload/{offload_id} | Get Offloaded Content|
...
|[**uploadMemoryNodeDocument**](#uploadmemorynodedocument) | **POST** /v2/memory/upload/{cat_id} | Upload Memory Node Document|
...
# **getOffloadedContent**
> string getOffloadedContent(getNodeRequest)
...
let nodeId: string; // (default to undefined)
let offloadId: string; // (default to undefined)
let workspaceId: string; // (default to undefined)
let getNodeRequest: GetNodeRequest; //

const { status, data } = await apiInstance.getOffloadedContent(
    nodeId,
    offloadId,
    workspaceId,
    getNodeRequest
...
| **getNodeRequest** | **GetNodeRequest**|  | |
| **nodeId** | [**string**] |  | defaults to undefined|
| **offloadId** | [**string**] |  | defaults to undefined|
| **workspaceId** | [**string**] |  | defaults to undefined|
...
> MemoryDocumentUploadResponse uploadMemoryNodeDocument()
...
const { status, data } = await apiInstance.uploadMemoryNodeDocument(
    catId,
    workspaceId,
    file
);
