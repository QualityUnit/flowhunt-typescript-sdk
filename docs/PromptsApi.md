...
|[**optimizePrompt**](#optimizeprompt) | **POST** /v2/prompts/optimize | Optimize Prompt|
...
# **optimizePrompt**
> OptimizedPromptResponse optimizePrompt(promptOptimizerRequest)
...
let workspaceId: string; // (default to undefined)
let promptOptimizerRequest: PromptOptimizerRequest; //

const { status, data } = await apiInstance.optimizePrompt(
    workspaceId,
    promptOptimizerRequest
);
```
...
|Name | Type | Description  | Notes|
|------------- | ------------- | ------------- | -------------|
| **promptOptimizerRequest** | **PromptOptimizerRequest**|  | |
| **workspaceId** | [**string**] |  | defaults to undefined|
